...
__NO_RETURN __STATIC_INLINE void __SW_SystemReset(void) {
  __DSB();                                                                                              /* Ensure all outstanding memory accesses including
                                                                       buffered write are completed before reset */
  SCB->AIRCR = (uint32_t)((0x5FAUL << SCB_AIRCR_VECTKEY_Pos) | (SCB->AIRCR & SCB_AIRCR_BFHFNMINS_Msk) | /* Keep BFHFNMINS unchanged. Use this Reset function in case your case need to keep it */
